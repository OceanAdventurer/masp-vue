  ...
  <div class="tree-expand">
    ...
    <div class="tree-btn-group" v-show="showTreeMore">
      ...
      <div class="tree-btn">
        ...
        <!-- 筛选方式 -->
        <div v-show="data.CHILDREN && (data.NAME === '筛选方式' || data.SUPER_NAME === '筛选方式') " class="tree-info" @click.stop="openTwoMenu(data, node, 'paramTwo_edit_filter')">
          <div class="tree-img-set tree-btn-filter"></div>
          <div class="tree-label" title="添加筛选方式">添加筛选方式</div>
        </div>
        <!-- 时间点 -->
         <div v-show="data.CHILDREN && (data.NAME === '时间点' || data.SUPER_NAME === '时间点')" class="tree-info" @click.stop="openTwoMenu(data, node, 'paramTwo_edit_timePoint')">
           <div class="tree-img-set tree-btn-point"></div>
           <div class="tree-label" title="添加时间点">添加时间点</div>
           </div>
        <!-- 时间段 -->
        <div v-show="data.CHILDREN  && (data.NAME === '时间段' || data.SUPER_NAME === '时间段')" class="tree-info" @click.stop="openTwoMenu(data, node, 'paramTwo_edit_timeSlot')">
          <div class="tree-img-set tree-btn-slot"></div>
          <div class="tree-label" title="添加时间段">添加时间段</div>
        </div>
        <!-- 度量值/测量值 -->
        <div v-show="data.CHILDREN  && (data.NAME === '测量值' || data.SUPER_NAME === '测量值')" class="tree-info" @click.stop="openTwoMenu(data, node, 'paramTwo_edit_measureValue')">
          <div class="tree-img-set tree-btn-metric"></div>
          <div class="tree-label" title="添加测量">添加测量值</div>
        </div>
        <!-- 逻辑值 -->
        <div v-show="data.CHILDREN && (data.NAME === '逻辑值' || data.SUPER_NAME === '逻辑值') " class="tree-info" @click.stop="openTwoMenu(data, node, 'paramTwo_edit_logicalValue')">
          <div class="tree-img-set tree-btn-boolean"></div>
          <div class="tree-label" title="添加逻辑">添加逻辑值</div>
        </div>
        <!-- 事件 -->
        <div v-show="data.CHILDREN && (data.NAME === '事件' || data.SUPER_NAME === '事件') " class="tree-info" @click.stop="openTwoMenu(data, node, 'paramTwo_edit_event')">
...
<!--        </div>-->
        <!--删除分析函数-->
        <div v-show="!data.CHILDREN && isCurrentUser" class="tree-info" @click.stop="openTwoMenu(data, node, 'paramTwo_delete')">
          <div class="tree-img-set tree-btn-delete"></div>
          <div class="tree-label" title="删除">删除</div>
        </div>
        <!--开始计算-->
        <div v-show="!data.CHILDREN && !data.LINK_DIMENSION_ID && isCurrentUser" class="tree-info" @click.stop="openTwoMenu(data, node, 'paramTwo_edit_compute')">
          <div class="tree-img-set compute-start"></div>
          <div class="tree-label" title="开始计算">开始计算</div>
        </div>
        <div v-show="!data.CHILDREN && !data.LINK_DIMENSION_ID && isCurrentUser" class="tree-info" @click.stop="openTwoMenu(data, node, 'paramTwo_share')">
        <div class="tree-img-set tree-btn-share"></div>
        <div class="tree-label" title="分享">分享</div>
      </div>
      <div v-show="!data.CHILDREN && !data.LINK_DIMENSION_ID && isCurrentUser" class="tree-info" @click.stop="openTwoMenu(data, node, 'paramTwo_clean')">
        <div class="tree-img-set tree-btn-share"></div>
        <div class="tree-label" title="配置清洗规则">配置清洗规则</div>
      </div>
    </div>
    </div>
