<style scoped>
  ...
  .left_tree_time_point::-webkit-scrollbar {
    width: 10px;
    height: 5px;
  }

  .left_tree_time_point::-webkit-scrollbar-thumb {
    border-radius: 10px;
...
<template>
  <div style="height: 100%;" v-loading="measureLoading">
    <!-- 新建分析参数算法之测量值 -->
    <div class="library-new-top" id="paramTwoLibraryMeasure">
      <div class="library-new-left" >
        <el-tabs v-model="activeName" @tab-click="handleClick" style="height: 100%;">
            ...
            <span v-show="childCreatorList === null || childCreatorList === undefined || childCreatorList.length === 0 ">
              <span v-if="creatorId === '9' || creatorId === 9">
               <ParamTwoSetTimePoint :formDt="formData"></ParamTwoSetTimePoint>
              </span>
              <span v-else-if="creatorId === '10' || creatorId === 10">
                <LibraryMeasureOneParam :formDt="formData"></LibraryMeasureOneParam>
              </span>
              <span v-else-if="creatorId === '11' || creatorId === 11">
...
export default {
  ...
  mounted () {
    ...
    // 接收 测量值中 时间点页面的值
    this.$bus.$on('paramTwoSetTimePointForLibraryMeasure', (obj) => {
      this.formData.timePoint = obj.timePoint
    })
    // 接收 测量值中 工程参数页面的值
    this.$bus.$on('paramTwoSetTimePointForLibraryMeasureColumnName', (obj) => {
      this.formData.columnName1 = obj.columnName1
    })
    this.$bus.$on('LibraryMeasureOneParamTime1', (obj) => {
      this.formData.timePointOne = obj.timePointOne
    })
    this.$bus.$on('LibraryMeasureOneParamTime2', (obj) => {
      this.formData.timePointTwo = obj.timePointTwo
    })
    this.$bus.$on('LibraryMeasureOneParamColumn', (obj) => {
      this.formData.columnName = obj.columnName
    })
    this.$bus.$on('LibraryMeasureOneParamaggregate', (obj) => {
      this.formData.aggregate = obj.aggregate
    })
  },
  methods: {
    // 查询子节点
    ...
    setVal (data, json) {
      ...
      if (this.creatorId === '10' || this.creatorId === 10) { // 选中10 的数据
      ...
      } else if (this.creatorId === '9' || this.creatorId === 9) {
        this.formData.timePoint = json.timepoint
        this.formData.columnName1 = json.columnName
      } else if (this.creatorId === '11' || this.creatorId === 11) {
         this.showRadio = this.creatorId * 1 // 选中radio
        checkEditor(this, data.twoDimensionContent) // 脚本内容
    ...
    // 选中 radio
    checkedRadio () {
      console.log('this.arrCreatorId' + this.arrCreatorId)
    ...
    previousClick () {
      ...
      this.attrExplain = ''
      this.clearData() // 点击上一步的时候清空所有输入的值 .下面会重新根据id查询一次
        ...
        this.$axios.get('/apm/getChildCreator', {params: {id: this.creatorId}}).then(response => {
          if (response.data.status !== null && response.data.status !== '' && response.data.status === 'E1001') {
            ...
            if (this.childCreatorList !== null && this.childCreatorList.length > 0) {
              ...
              this.getTree()
              this.showSaveBtn = true
              if (this.creatorId === '11' || this.creatorId === 11) {
    ...
    saveMeasure () {
      ...
      if (this.creatorId === 9 || this.creatorId === '9') {
        paramStr = this.$qs.stringify({
          ...
          json: '{"TIMEPOINT": "' + this.formData.timePoint + '", "COLUMN_NAME": "' + this.formData.columnName1 + '"}',
          steps: '{"timepoint": "' + this.formData.timePoint + '", "columnName": "' + this.formData.columnName1 + '", "arrCreatorId": ' + JSON.stringify(this.arrCreatorId) + ', "scriptTypes": ' + JSON.stringify(this.scriptTypes) + ',  "activeName": "' + this.activeName + '", "creatorId": "' + this.creatorId + '", "description": "' + this.formLabelAlign.twoDimensionDescription + '", "stepTitle": "' + this.stepTitle + '"}'
        })
      } else if (this.creatorId === 10 || this.creatorId === '10') {
        paramStr = this.$qs.stringify({
          twoDimensionScriptId: this.scriptId,
          name: this.formLabelAlign.twoDimensionName,
          description: this.formLabelAlign.twoDimensionDescription,
          ...
          creatorId: this.creatorId,
          type: this.scriptType,
          json: '{"TIME_POINT_START": "' + this.formData.timePointOne + '", "TIME_POINT_END": "' + this.formData.timePointTwo + '", "COLUMN_NAME": "' + this.formData.columnName + '", "AGGREGATE":"' + this.formData.aggregate + '"}',
          steps: '{"timepoint1": "' + this.formData.timePointOne + '", "timepoint2": "' + this.formData.timePointTwo + '", "arrCreatorId": ' + JSON.stringify(this.arrCreatorId) + ', "scriptTypes": ' + JSON.stringify(this.scriptTypes) + ', "columnName": "' + this.formData.columnName + '","aggregate": "' + this.formData.aggregate + '", "activeName": "' + this.activeName + '", "creatorId": "' + this.creatorId + '", "description": "' + this.formLabelAlign.twoDimensionDescription + '", "stepTitle": "' + this.stepTitle + '"}'
        })
      } else if (this.creatorId === 11 || this.creatorId === '11') {
