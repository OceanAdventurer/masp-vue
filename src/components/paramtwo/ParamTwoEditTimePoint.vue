            ...
            <div class="param-two-time-point" v-show="showContentPage">
              ...
              <div class="param-two-time-point-left">
                ...
                <div class="left_filter" v-show="searchShow">
                  <el-input placeholder="请输入关键字搜索" size="small" v-model="filterText">
              ...
              <span v-else-if="creatorId === 7 || creatorId === '7'">
                <!-- <iframe id="ifid" ref="iframe" src="/csap/static/groovyIDE/index2.html" width="100%" height="100%"></iframe> -->
                <!-- 左侧元素 -->
...
export default {
  ...
  methods: {
      ...
      this.$axios.get('/apm/getTwoDimensionScriptById', {params: {id: this.scriptId}}).then(response => {
        ...
        if (data.status !== null && data.status !== '' && data.status === 'E1001') {
          ...
          this.formLabelAlign.attributeName = data.name
           // 脚本编辑的值 如果没有则为null
          if (data.createSteps !== null && data.createSteps !== '' && data.createSteps !== undefined) {
            ...
            if (json !== null && json !== undefined) {
              ...
              if (this.creatorId === '7' || this.creatorId === 7) {
    ...
    previousClick () {
      ...
      this.getChildCreatorByScriptType(scriptType) // 重新加载值
    ...
    onSubmit () {
      if (this.creatorId === 6 || this.creatorId === '6') {
        this.saveTimePoint()
      } else if (this.creatorId === 7 || this.creatorId === '7') {
        this.saveEditor()
      }
    },
    // 测试事件--加载profileId
