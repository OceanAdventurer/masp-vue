      ...
      <div class="tree-btn">
        ...
        <div>
          ...
          <div v-show="data.CHILDREN" class="tree-info" @click.stop="addTreeNode(data, node, 'node')">
            ...
            <div class="tree-label" title="添加节点">添加节点</div>
          </div>
          <!-- 添加子节点 -->
          <div v-show="!data.CHILDREN" class="tree-info" @click.stop="createAnalysis(data, node)">
            <div class="tree-img-set tree-btn-plus-c"></div>
            <div class="tree-label" title="新建挖掘">新建挖掘</div>
          </div>
          <!-- 暂时不支持编辑节点 -->
          <div v-show="!data.CHILDREN" class="tree-info" @click.stop="editTreeNode(data, node)">
            <div class="tree-img-set tree-btn-rnme"></div>
            <div class="tree-label" title="">重命名</div>
          </div>
          <!-- 树根不允许删除，暂时也不支持删除节点 -->
          <div v-show="isShowDelete" class="tree-info" @click.stop="delTreeNode(data, node)">
...
export default {
  ...
  computed: {
    ...
    showTreeMore: function () {
      if (!this.data.status && this.isShowMore) { // 节点不处于编辑状态,同时显示按钮标识为true
  ...
  methods: {
    ...
    createAnalysis (data, node) {
      ...
      this.$bus.$emit('dataMiningAddTab', {enName: 'dataMining_file_new', zhName: '新建挖掘', isClosable: true, parent: 'dataMining_mining', count: 0})
    },
