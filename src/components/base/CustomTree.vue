      ...
      <div class="tree-btn">
        ...
        <div v-if="type == 'paramone'">
          ...
          <div v-show="data.CHILDREN" class="tree-info" @click.stop="addTreeNode(data, node, 'node')">
            ...
            <div class="tree-label" title="添加工程参数">添加工程参数</div>
          </div>
          <!-- 暂时不支持编辑节点 -->
...
<!--          <div v-show="!data.CHILDREN" class="tree-info" @click.stop="openTwoMenu(data, node, 'create')">-->
<!--            <div class="tree-img-set tree-btn-add-suanfa"></div>-->
<!--            <div class="tree-label" title="新建算法">新建算法</div>-->
<!--          </div>-->
<!--          <div v-show="!data.CHILDREN " class="tree-info" @click.stop="openTwoMenu(data, node,'edit')">-->
<!--            <div class="tree-img-set tree-btn-edit"></div>-->
<!--            <div class="tree-label" title="编辑算法">编辑算法</div>-->
<!--          </div>-->
<!--          <div v-show="!data.CHILDREN" class="tree-info" @click.stop="openTwoMenu(data, node, 'clear')">-->
...
<!--          </div>-->
        ...
        <div v-else-if="type == 'paramtwo'">
          ...
          <div v-show="data.CHILDREN" class="tree-info" @click.stop="addTreeNode(data, node, 'node')">
            <div class="tree-img-set tree-btn-plus-c"></div>
            <div class="tree-label" title="添加分析参数">添加分析参数库</div>
          </div>
          <!-- 删除目录 -->
          <div v-show="data.CHILDREN && data.TYPE === 'USERCHILDNODE'" class="tree-info" @click.stop="delTreeNode(data, node, 'catalog')">
          <!-- <div class="tree-info" @click.stop="delTreeNode(data, node, 'catalog')"> -->
            <div class="tree-img-set tree-btn-delete"></div>
            <div class="tree-label" title="删除">删除</div>
          </div>
          <!-- 暂时不支持编辑节点 -->
          <div v-show="!data.CHILDREN && isShowEdit && userInfo.userName === data.CREATE_USER" class="tree-info" @click.stop="editTreeNodeTwo(data, node)">
            <div class="tree-img-set tree-btn-edit"></div>
            <div class="tree-label" title="编辑">编辑</div>
          </div>
          <div v-show="!data.CHILDREN && isShowEdit && userInfo.userName !== data.CREATE_USER" class="tree-info" @click.stop="lookParam(data, node)">
            <div class="tree-img-set tree-btn-edit"></div>
            <div class="tree-label" title="编辑">查看</div>
          </div>
          <div v-show="!data.CHILDREN && isShowEdit && userInfo.userName === data.CREATE_USER" class="tree-info" @click.stop="renameTreeNode(data, node, 'node')">
            <div class="tree-img-set tree-btn-rnme"></div>
            <div class="tree-label" title="">重命名</div>
          </div>
          <!-- 删除分析参数库 -->
          <div v-show="!data.CHILDREN && userInfo.userName === data.CREATE_USER" class="tree-info" @click.stop="delTreeNode(data, node, 'node')">
            <div class="tree-img-set tree-btn-delete"></div>
            <div class="tree-label" title="删除">删除</div>
          </div>
        </div>
        <div v-else>
          ...
          <div v-show="data.CHILDREN" class="tree-info" @click.stop="addTreeNode(data, node, 'catalog')">
            <div class="tree-img-set tree-btn-plus-p"></div>
            <div class="tree-label" title="添加分析目录">添加分析目录</div>
          </div>
          <!-- 添加子节点 -->
          <div v-show="data.CHILDREN" class="tree-info" @click.stop="addTreeNode(data, node, 'node')">
            <div class="tree-img-set tree-btn-plus-c"></div>
            <div class="tree-label" title="添加分析节点">添加分析节点</div>
          </div>
          <!-- 添加子节点 -->
          <div v-show="!data.CHILDREN" class="tree-info" @click.stop="createAnalysis(data, node, 'analysis')">
            <div class="tree-img-set tree-btn-plus-c"></div>
            <div class="tree-label" title="新建分析">新建分析</div>
          </div>
          <div v-show="!data.CHILDREN" class="tree-info" @click.stop="createAnalysis(data, node, 'event')">
            <div class="tree-img-set tree-btn-plus-c"></div>
            <div class="tree-label" title="新建事件分析">新建事件分析</div>
          </div>
          <div v-show="!data.CHILDREN" class="tree-info" @click.stop="createAnalysis(data, node, 'dhbcsdb')">
            <div class="tree-img-set tree-btn-plus-c"></div>
            <div class="tree-label" title="新建多航班参数对比">新建多航班参数对比</div>
          </div>
          <!-- 暂时不支持编辑节点 -->
          <div v-show="node.level !== 1 && isShowEdit && data.TREETYPE === 'USERCHILDNODE'" class="tree-info" @click.stop="editTreeNode(data, node)">
            <div class="tree-img-set tree-btn-rnme"></div>
            <div class="tree-label" title="">重命名</div>
          </div>
          <!-- 树根不允许删除，暂时也不支持删除节点 -->
          <div v-show="node.level !== 1 && isShowDelete && data.TREETYPE === 'USERCHILDNODE'" class="tree-info" @click.stop="delTreeNode(data, node)">
            <div class="tree-img-set tree-btn-delete"></div>
            <div class="tree-label" title="删除">删除</div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    showTreeMore: function () {
      if (this.type === 'paramtwo' && this.data.CHILDREN && this.data.TYPE !== 'USERNODE' && this.data.TYPE !== 'USERCHILDNODE') {
        return false
      } else if (this.type !== 'paramone' && this.type !== 'paramtwo' && this.data.TREETYPE !== 'USERNODE' && this.data.TREETYPE !== 'USERCHILDNODE') {
        return false
  ...
  methods: {
    ...
    renameTreeNode (data, node, type) { // 重命名
      console.log('renameTreeNodeHandel', data, node, type)
      this.$emit('renameTreeNodeHandle', data, node, type)
    },
    createAnalysis (data, node, type) {
      this.$store.commit('TEMP_MANAGER_TREE_NODE_ID', data.ID)
      if (type === 'event') {
        this.$bus.$emit('openEventDialogHandle', data, node)
      } else if (type === 'dhbcsdb') {
        this.$bus.$emit('addTab', 'analysis_dhbcsdb', '新建多航班参数对比', 'analysis_file', 'analysis')
      } else {
        this.$bus.$emit('addTab', 'analysis_file_new', '新建分析', 'analysis_file', 'analysis')
      }
    },
    editTreeNodeTwo (data, node) { // 参数TWO 编辑
