...
<script>
import EventTree from 'components/base/EventTree'
...
export default {
  ...
  mounted () {
    this.$bus.$on('EventManagerEmit', (enName) => {
      switch (enName) {
        case 'event_add_catalog' : // 添加目录
          this.addNodeFun(this.treeData, this.treeNode, 'catalog')
          break
        case 'event_profile_rename' : // 重命名
          this.renameNodeFun(this.treeData, this.treeNode, 'node')
          break
        case 'event_delete_catalog' : // 删除目录
          this.delNodeFun(this.treeData, this.treeNode, 'catalog')
          break
        case 'event_library_add' : // 添加事件库节点
          this.addNodeFun(this.treeData, this.treeNode, 'node')
          break
        case 'event_delete_profile' : // 删除事件库节点
          this.delNodeFun(this.treeData, this.treeNode, 'node')
          break
      }
    })
    this.$nextTick(() => {
  ...
  methods: {
    ...
    getTree () {
      ...
      this.$axios.get('/apm/getEventProfileTree', {params: {parentId: this.parentId, searchContent: this.filterText}}).then(response => {
        this.treeLoading = false
        this.fileNewTreeArr = []
        this.loading = false
        var data = response.data
        this.fileNewTreeArr = data.data
        if (this.openTreeId && this.addStauts) { // 保存后的刷新
          this.expandedKeys = this.openTreeId
          this.addStauts = false // 改为 false
        } else if (data.defaultNode) {
    ...
    renderContent (h, {node, data, store}) {
      ...
      return h(EventTree, {
        ...
        on: { // 绑定方法 data node store type是当前节点信息
          ...
          lookEventNodeHandle: ((data, node, type) => that.lookParam(data, node, type))
        }
      })
    },
    // 增加节点,且是处于编辑中
    addNodeFun (data, node, type) {
      this.openTreeId = data.ID
    ...
    editTreeData (data) {
      var url = ''
      var paramStr = {}
      url = '/apm/renameEventProfile'
      paramStr = {id: data.ID, name: data.NAME}
    ...
    addTreeData (data, parentNode) {
      ...
      if (data.nodeType === 'catalog') {
        url = '/apm/addEventProfileTree'
        paramStr = {parentId: parentNode.ID, name: data.NAME}
      } else {
        url = '/apm/addEventProfile'
    ...
    delNodeFun (data, node, type) {
      this.openTreeId = data.PARENT_ID
      this.$confirm('您确定要删除吗?', '提示', {
        ...
        if (type === 'catalog') { // 删除目录
          this.$axios.get('/apm/deleteEventProfileTree', {params: {id: data.ID}}).then(response => {
          ...
          this.$axios.get('/apm/deleteEventProfile', {params: {profileId: data.ID}}).then(response => {
            let respData = response.data
            if (respData.status === '0') { // 成功
              this.$message({
                type: 'success',
    ...
    // 子节点击事件 加载中间及右侧数据getProfileEventLevel?id=3ae4a2c1ac0a41f6af1097de2b6583d4
    handleNodeClick (data, node, nodeCommpent) {
      ...
      if (!data['CHILDREN'] && data.NAME !== null) {
        ...
        if (data.CREATE_USER && this.userInfo.userName.toUpperCase() === data.CREATE_USER.toUpperCase()) {
          hideMenuObj.event_delete_profile = true // 开启删除
          hideMenuObj.event_profile_edit = true // 开启编辑
          hideMenuObj.event_profile_rename = true // 开启重命名
        } else {
          hideMenuObj.event_profile_look = true // 开启查看
        }
        // this.$bus.$emit('eventAddTabInnerTree', {parentId: data.ID})
      } else { // 父节点
        this.IsShowTab = false
        this.profileObj = [] // 清空数据
        //默认点击父节点都显示
        hideMenuObj.event_add_catalog = true
        hideMenuObj.event_delete_catalog = true
        hideMenuObj.event_library_add = true
        if (data.TYPE === 'USERNODE') { // 只能创建不能删除 禁用删除
          disabledMenuObj.event_delete_catalog = true
        } else if (data.TYPE === 'COMMONTREE' || data.PARENT_ID === '0' || data.PARENT_ID === 0) { // 公共树不能操作
          disabledMenuObj.event_add_catalog = true
          disabledMenuObj.event_delete_catalog = true
          disabledMenuObj.event_library_add = true
        }
      }
      this.hideMenuObj = hideMenuObj // 保存变量
      this.disabledMenuObj = disabledMenuObj
      this.$bus.$emit('eventParamLibrayAddCatalog1Vaue', data) // 传值过去，然后点击二级菜单，触发对应的事件要用到
      this.$bus.$emit('eventParamLibrayAddCatalog1VaueManager', data, node, {treeData: this.fileNewTreeArr}) // 传值过去，然后点击二级菜单，触发对应的事件要用到
    ...
    getDetail (val) {
      ...
      this.$axios.get('/apm/getEventProfileDetail', {params: {id: val}}).then(response => {
        this.tableLoading = false
        this.profileObj = []
        ...
        if (data.status !== null && data.status !== '' && data.status === 'E1001') {
          ...
          this.$axios.get('/apm/getProfileEventLevel', {params: {id: val}}).then(response => {
            let d = response.data
            let level = ''
            for (let i = 0; i < d.length; i++) {
              if (!level) {
                level = d[i]['EVENT_CAPTION']
              } else {
                level += ',' + d[i]['EVENT_CAPTION']
              }
            }
            ...
            this.profileObj.push({ note: '等级', name: level })
    ...
    // 点击编辑按钮触发事件，显示内层树
    lookParam (data, node, type) {
      if (type === 'edit') {
          this.$bus.$emit('eventParamLibrayAddCatalog1Vaue', data) // 传值过去，然后点击二级菜单，触发对应的事件要用到
          this.$bus.$emit('paramEventAddTab', {enName: 'event_profile_edit', zhName: data.NAME, isClosable: true, parent: 'paramEvent_eventLibrary', count: 0})
      } else {
          this.$bus.$emit('eventParamLibrayAddCatalog1Vaue', data) // 传值过去，然后点击二级菜单，触发对应的事件要用到
          this.$bus.$emit('paramEventAddTab', {enName: 'event_profile_look', zhName: data.NAME, isClosable: true, parent: 'paramEvent_eventLibrary', count: 0})
      }
    },
    changeOperAtion () {
      if (this.operation) {
        ...
        this.$confirm('您确定要开启吗?', '提示', {
          ...
          this.operation = true
          this.$axios.get('/apm/activeEventProfile', {params: {id: this.id}}).then(response => {
            var data = response.data
            if (data.status !== null && data.status !== '' && data.status === 'E1001') {
              this.$bus.$emit('logBackInHandle', response.data)
        ...
        this.$confirm('关闭将清空该profile所有计算结果，是否关闭？', '提示', {
          ...
          this.operation = false
          this.$axios.get('/apm/inActiveEventProfile', {params: {id: this.id}}).then(response => {
            var data = response.data
            if (data.status !== null && data.status !== '' && data.status === 'E1001') {
              this.$bus.$emit('logBackInHandle', response.data)
            } else if (data.status === '0') {
